    ...
    return (
      <div className={`media ${audio.spam && 'spam'} ${audio.featured && 'featured'}`} key={audio._id}>
        <div className="media-left">
          ...
          {
            showMore && 
            <a href={audio.file} target="_blank"><i className="mt-2 fas fa-download fa-fw is-size-4"></i></a>
          }
        </div>
        <div className="media-body">
          ...
          <p className="my-0" onClick={this.toggleShowMore} style={{cursor: 'pointer'}}>
          {
          ...
          {
            audio.user ?
             <span><span className="upper">{audio.user.lastName}</span>, {audio.user.firstName} / {audio.user.email} / DNI {audio.user.dni}</span>
             : <span><span>{audio.fullName || '(Sin completar)' }</span> / <span>{audio.email || '(Sin completar)'}</span> / <span>DNI {audio.dni || '(Sin completar)'}</span> <span className="text-warning is-size-7">(No registrado)</span></span>
          }
          ...
          </p>
          {
            ...
            <div className="">
              ...
              <p className="my-0 description">Escala: {audio.scale || '(Sin completar)'} / Localidad: {audio.location || '(Sin completar)'} / Contacto: {audio.contact || '(Sin completar)'}</p>
        ...
        <div className="media-right">
          <p className="reaction text-center is-size-5 mb-0">
            ...
            {
              !showMore && audio.subgroup && <span className="badge badge-info px-1 pt-0 pb-0 mx-1" style={{fontWeight: 300, borderRadius: 3, lineHeight: 'normal'}}>{audio.subgroup}</span>
            }
