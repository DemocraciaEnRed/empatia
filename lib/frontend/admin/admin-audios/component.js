...
export default class AdminAudios extends Component {
  ...
  loadAudios (query) {
    ...
    window.fetch(`/api/v2/audio/admin/all/${this.state.page}?orderBy=${this.state.orderCreatedAt}&limit=${this.state.pageSize}${queryString ? '&' : ''}${queryString || ''}`, {
      method: 'GET',
      credentials: 'include',
      headers: {
  ...
  goBack () {
    if (this.state.page > 0) {
      this.setState({
        ...
        this.loadAudios(this.state.queryString)
      })
    }
  }
          ...
          <div className="col-md-4">
            <div className="form-group">
              ...
              <select className="form-control" name="orderCreatedAt" onChange={this.handleInputChange}>
                ...
                <option value="desc">Más antiguos primero</option>
              </select>
            </div>
          </div>
