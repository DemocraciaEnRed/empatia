...
import ForumTopicCards from '../../forum-topic-cards/component'
...
export default class Header extends Component {
  render () {
    ...
    if (authorData) {
      ...
      author = (
        <span className='author'><b>{t('admin-topics-form.label.author')}</b>:
          &nbsp;{authorName}
        </span>
      )
    }
    ...
    return (
      <header className={`topic-article-header topic-article-content ${this.props.closed ? 'isclosed' : ''}`}>
        ...
        {closingAt} */}
        {
          this.props.tag &&
          <div>
            <span className='text-main is-size-5' style={{borderRadius: '5px', fontWeight: '800', textTransform: 'uppercase'}}>{this.props.tag.name}</span>
          </div>
        }
        <h1 className="title my-4">{this.props.mediaTitle}</h1>
