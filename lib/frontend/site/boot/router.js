...
import SetPassword from 'lib/frontend/site/reset-for-setpassword/component'
...
export default {
  ...
  childRoutes: [
    ...
    },
    {
      ...
      indexRoute: { component: Forgot },
      childRoutes: [
        { path: 'reset/:token', component: SetPassword }
      ]
    },
...
function checkRestrictAnon (nextState, replace, next) {
  user.fetch().then(() => {
    ...
    // console.log('allow?', config.allowPublicSignUp)
    // console.log('check', user.state.value.staff || user.state.value.manager)
    if (!user.state.rejected && this.path == 'signup' && !config.allowPublicSignUp && (user.state.value.staff || user.state.value.manager)) {
      return next()
    }
