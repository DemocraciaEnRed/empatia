...
export class UserBadge extends Component {
  ...
  render () {
    ...
    // console.log(userAttrs.staff || userAttrs.manager)
    if (userAttrs && userAttrs.privileges && userAttrs.privileges.canManage) {
      ...
      if (!config.allowPublicSignUp && (userAttrs.staff || userAttrs.manager)) {
    ...
    return (
      <div className={classes.join(' ')} onClick={this.toggleMenu}>
        <button className='header-link'>
          ...
          {
            userAttrs &&
            <span className='name'>{userAttrs.firstName}</span>
          }
