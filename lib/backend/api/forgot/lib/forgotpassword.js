...
exports.setPassword = function resetPassowrd (formData, callback) {
  log('password set requested. token : [%s]', formData.token)
  exports.verifyToken(formData.token, function (err, token) {
    if (err) return callback(err)
    log('password set requested. token : [%s]. token verified', formData.token)
    api.user.get(token.user, function (err, user) {
      if (err) return callback(err)
      log('about to set password. user : [%s].', user.id)
      user.setPassword(formData.password, function (err) {
        if (err) return callback(err)
        log('password seted, but user not saved yet. user : [%s].', user.id)
        user.save(function (err) {
          if (err) return callback(err)
          log('Saved user [%s]', user.id)
          token.remove(function (err) {
