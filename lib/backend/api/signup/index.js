...
function (req, res, next) {
  var profile = {
    email: req.body.answers.find(a => a.field.ref === 'email').email,
    firstName: req.body.answers.find(a => a.field.ref === 'firstName').text,
    lastName: req.body.answers.find(a => a.field.ref === 'lastName').text,
    // number to string because typeform returns a number
    dni: (req.body.answers.find(a => a.field.ref === 'dni').number).toString()
  }
