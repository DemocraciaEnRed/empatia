...
app.post('/audio/submit',
  ...
  // validate captcha
  (req, res, next) => {
    validateCaptcha(req.body.captchaResponse)
      .then(() => {
        next()
      })
      .catch((err) => {
        log(`Error validating captcha`)
        res.send({
          status: false,
  ...
  async function uploadToS3 (req, res, next) {
    ...
    try {
      if (!req.files) {
        ...
        let audioName = `${moment(new Date()).format('YYYYMMDD-HHmmss')}-${req.user.dni}.mp3`
          ...
          s3.upload({
            ...
            if (err) {
              ...
              let audioData = {
                ...
                size: audio.size,
                fullName: req.user ? req.user.fullName : (req.body.fullname ? req.body.fullname : null),
                email: req.user ? req.user.email : (req.body.email ? req.body.email : null),
                dni: req.user ? req.user.dni : (req.body.dni ? req.body.dni : null),
                province: req.user ? req.user.provincia : (req.body.province ? req.body.province : null),
                location: req.user ? req.user.localidad : (req.body.location ? req.body.location : null),
                contact: req.user ? req.user.contacto : (req.body.contact ? req.body.contact : null)
              }
...
//                 dni: req.user.dni,
//                 province: req.user.provincia,
//                 location: req.user.location,
//                 contact: req.user.contacto
//               }
//               dbApi.audio.create(audioData)
//               .then((savedAudio) => {
//                 // delete temp file
...
app.post('/audio/description',
  ...
  async function submitAudio (req, res, next) {
    ...
    try {
      let theAudio = await dbApi.audio.get(req.body.id)
      if (!theAudio) {
        res.send({
          status: false,
          message: 'Audio not found'
